...
def _bpl_w07(x, F_bpl, T_bpl, alpha_1, alpha_2, F_a, T_a, alpha_a, t_a):
    if T_bpl > t_a:
        tmp=T_bpl
        T_bpl=t_a
        t_a=tmp
    ...
    bpl1 = lambda x: F_bpl - alpha_1 * (x - T_bpl)
    bpl2 = lambda x: F_bpl - alpha_2 * (x - T_bpl)

    w07_rise = lambda x: (
        -10 ** (t_a-x) + alpha_a - alpha_a * 10 ** (x - T_a) + F_a * np.log(10)
    ) / np.log(10)

    w07_decay = lambda x: F_a + T_a * alpha_a - x * alpha_a - 10 ** (t_a-x) / np.log(10)

    intersection = lambda x: bpl2(x)-w07_rise(x)
    tt = sp.optimize.root_scalar(intersection, method = 'secant', x0 = T_bpl).root

    cond = [(x < T_bpl), (x >= T_bpl)*(x <= tt), (x > tt)*(x <= T_a), (x >= T_a)]
    func = [bpl1, bpl2, w07_rise, w07_decay]
    vals = np.piecewise(x, cond, func)
...
def _w07_pl(x, F_p, alpha_p, t, alpha_1, t0, t1):
    ...
    w07_decay = lambda x: F_p + t0 * alpha_p - x * alpha_p - 10 ** (t-x) / np.log(10)
    pl = lambda x: F_a - alpha_1 * (x - t1)

    intersection = lambda x: w07_decay(x)-pl(x)
    tt = sp.optimize.root_scalar(intersection, method = 'secant', x0 = t1).root

    cond = [(x < t0), x < tt, (x >= t1)]
    func = [w07_rise, w07_decay, pl]
    vals = np.piecewise(x, cond, func)
...
#  Prompt as simple broken power law

def _bpl_pl(x, F_p, alpha1, alpha2, alpha_p, t0, t1):
    if t0 > t1:
        tmp = t0
        t0 = t1
        t1 = tmp
#    F_d = F_p - alpha2 * (t1 - t0)
    F_a = F_p - alpha1 * t0 - alpha2 * (t1 - t0)
    before = lambda x: F_p - alpha1 * (x - t0)
    middle = lambda x: F_p - alpha2 * (x - t0)
    after = lambda x: F_a - alpha_p * (x - t1)
    vals = np.piecewise(x, [x < t0, (x >= t0) * (x < t1), x >= t1], [before, middle, after])
    return vals

def _bpl_bpl(x, F_p, alpha1, alpha2, alpha3, alpha4, t0, t1, t2):
    if t0 > t1:
        tmp = t0
        t0 = t1
        t1 = tmp
    ...
    if t0 > t1:
        ...
        t0 = t1
        t1 = tmp
#    F_d = F_p - alpha2 * (t2 - t1)
#    F_a = F_p - alpha2 * t1 - alpha3 * (t2 - t1)
    F_a = F_p - alpha1 * t0 - alpha2 * (t1 - t0)
    before = lambda x: F_p - alpha1 * (x - t0)
    middle = lambda x: F_p - alpha2 * (x - t0)
    middle2 = lambda x: F_a - alpha3 * (x - t2)
    after = lambda x: F_a - alpha4 * (x - t2)
    vals = np.piecewise(x, [x < t0, (x >= t0) * (x < t1), (x >= t1) * (x < t2), x >= t2], [before, middle, middle2, after])
    return vals

def _bpl_bpl_pl(x, F_p, alpha1, alpha2, alpha3, alpha4, alpha_p, t0, t1, t2, t3):
    if t0 > t1:
        tmp = t0
        t0 = t1
        t1 = tmp
    ...
    if t0 > t1:
        ...
        t1 = tmp
#    F_a = F_p - alpha2 * t1 - alpha3 * (t2 - t1)
#    F_d = F_a - alpha4 * (t3 - t2)
    F_a = F_p - alpha1 * t0 - alpha2 * (t1 - t0)
    F_d = F_a - alpha3 * (t2 - t1)
#    F_d2 = F_d - alpha4 * (t3 - t2)
    before = lambda x: F_p - alpha1 * (x - t0)
    middle = lambda x: F_p - alpha2 * (x - t0)
    middle2 = lambda x: F_a - alpha3 * (x - t2)
    middle3 = lambda x: F_a - alpha4 * (x - t2)
    after = lambda x: F_d - alpha_p * (x - t3)
    vals = np.piecewise(x, [x < t0, (x >= t0) * (x < t1), (x >= t1) * (x < t2), (x >= t2) * (x < t3), (x >= t3)], [before, middle, middle2, middle3, after])
    return vals

def _w07_w07(x, F_p0, alpha_0, T0, t0, F_p1, alpha_1, T1, t1):
    #Orders temporal variables
    if t0 > T0:
        tmp = t0
        t0 = T0
        T0 = tmp
    ...
    first_rise = lambda x: (
        -10 ** (t0-x) + alpha_0 - alpha_0 * 10 ** (x - T0) + F_p0 * np.log(10)
    ) / np.log(10)
    
    first_decay = lambda x: F_p0 + T0 * alpha_0 - x * alpha_0 - 10 ** (t0-x) / np.log(10)

    second_rise = lambda x: (
        -10 ** (t1-x) + alpha_1 - alpha_1 * 10 ** (x - T1) + F_p1 * np.log(10)
    ) / np.log(10)

    second_decay = lambda x: F_p1 + T1 * alpha_1 - x * alpha_1 - 10 ** (t1-x) / np.log(10)

    middle_difference = lambda x: first_decay(x)-second_rise(x)
    tt = sp.optimize.root_scalar(middle_difference, method = 'secant', x0 = T0).root

    cond = [(x < T0), x < tt, x >= tt, (x >= T1)]
    func = [first_rise, first_decay, second_rise, second_decay]
    vals = np.piecewise(x, cond, func)
    return vals


    
def _pl_w07_pl(x, F_p, alpha_p, alpha_a1, alpha_a2, t0, t1, t2, t):
    if t0 > t1:
        tmp=t0
        t0=t1
        t1=tmp
    ...
    before = lambda x: F_p - alpha_p*x
    middle = lambda x: (
        -10 ** (t-x) + alpha_a1 - alpha_a1 * 10 ** (x - t1) + F_a * np.log(10)
    ) / np.log(10)

    middle2 = lambda x: F_a + t1 * alpha_a1 - x * alpha_a1 - 10 ** (t-x) / np.log(10)
    after = lambda x: F_d - alpha_a2 * (x - t2)
    vals = np.piecewise(x, [x < t0, (x >= t0) * (x < t1), (x >= t1) * (x < t2), x >= t2], [before, middle, middle2, after])
    return vals

def _w07_w07_pl(x, F_p1, T_p1, alpha_p1, t_p1, F_p2, T_p2, alpha_p2, t_p2, F_pl, alpha_pl):
    if t_p1 > T_p1:
        tmp=t_p1
        t_p1=T_p1
        T_p1=tmp
    ...
    w07_rise1 = lambda x: (
        -10 ** (t_p1-x) + alpha_p1 - alpha_p1 * 10 ** (x - T_p1) + F_p1 * np.log(10)
    ) / np.log(10)

    w07_decay1 = lambda x: F_p1 + T_p1 * alpha_p1 - x * alpha_p1 - 10 ** (t_p1-x) / np.log(10)

    w07_rise2 = lambda x: (
        -10 ** (t_p2-x) + alpha_p2 - alpha_p2 * 10 ** (x - T_p2) + F_p2 * np.log(10)
    ) / np.log(10)

    w07_decay2 = lambda x: F_p2 + T_p2 * alpha_p2 - x * alpha_p2 - 10 ** (t_p2-x) / np.log(10)

    pl = lambda x: F_pl - alpha_pl * x

    w07_w07_intersection = lambda x: w07_decay1(x)-w07_rise2(x)
    tt1 = sp.optimize.root_scalar(w07_w07_intersection, method = 'secant', x0 = T_p1).root

    w07_pl_intersection = lambda x: w07_decay2(x)-pl(x)
    tt2 = sp.optimize.root_scalar(w07_pl_intersection, method = 'secant', x0 = T_p2).root

    # print(tt1, tt2)
    
    cond = [x < T_p1, x < tt1, x >= tt1, (x >= T_p2)*(x < tt2), x >= tt2]
    func = [w07_rise1, w07_decay1, w07_rise2, w07_decay2, pl]
    vals = np.piecewise(x, cond, func)
...
class Model:
    ...
    @classmethod
    def SIMPLE_BPL_W07(cls, vary_t=True):
        return cls(
            name="Simple broken power law + Willingale 2007",
            slug="bpl+w07",
            func=_bpl_w07,
            func_args=[
                Parameter(
                    "F_bpl",
                    "log flux at of bpl (log erg/cm^2/s)",
                    min=-18,
                    max=-2,
                    plot_fmt=r"$F_bpl$",
                ),
                ...
                Parameter(
                    "F_a",
                    "log flux at peak of afterglow (log erg/cm^2/s)",
                    min=-18,
                    max=-2,
                    plot_fmt=r"$F_a$",
                ),
                ...
                Parameter(
                    "t_a",
                    "log time at peak of w07 (log sec)",
                    min=0,
                    max=10,
                )
            ]
    ...
    def W07_POWER_LAW(cls, vary_t=True):
        return cls(
            ...
            func_args=[
                ...
                Parameter(
                    "t0",
                    "log time at the end of plateau (log sec)",
                    min=0,
                    max=5,
                ),
                Parameter(
                    "t1",
                    "log time at the end of willingale (log sec)",
                    min=0,
                    max=10,
                )
            ]
    ...
    def POWER_LAW_W07(cls, vary_t=True):
        return cls(
            ...
            func_args=[
                ...
                Parameter(
                    "F_a",
                    "log flux at the end of plateau (log erg/cm^2/s)",
                    min=-18,
                    max=-8,
                    plot_fmt=r"$F_a$",
                ),
    ...
    @classmethod
    def SIMPLE_BPL_POWER_LAW(cls):
        return cls(
            name="Simple broken power law + power law",
            slug="bpl+pl",
            func=_bpl_pl,
            func_args=[
                Parameter(
    ...
    @classmethod
    def SIMPLE_BPL_SIMPLE_BPL(cls):
        return cls(
            name="Simple broken power law + Simple broken power law",
            slug="bpl+bpl",
            func=_bpl_bpl,
            func_args=[
                Parameter(
                ...
                Parameter(
                    "alpha3",
                    "temporal decay index of middle2 power law",
                    min=0,
                    max=7,
                    plot_fmt=r"$\alpha3$",
                ),
                ...
                Parameter(
                    "t2",
                    "log time at beginning of afterglow (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t2$",
                )
    ...
    @classmethod
    def SIMPLE_BPL_SIMPLE_BPL_POWER_LAW(cls):
        return cls(
            name="Simple broken power law + Simple broken power law",
            slug="bpl+bpl+pl",
            func=_bpl_bpl_pl,
            func_args=[
                ...
                Parameter(
                    "alpha4",
                    "temporal decay index of middle3 power law",
                    min=0,
                    max=7,
                    plot_fmt=r"$\alpha4$",
                ),
                ...
                Parameter(
                    "t3",
                    "log time at end of afterglow (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t3$",
                )
    ...
    def DOUBLE_W07(cls):
        return cls(
            ...
            func_args=[
                ...
                Parameter(
                    "t0",
                    "log time at the first peak (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t0$",
                ),
                Parameter(
                    "F_p1",
                    "log flux at the end of afterglow (log erg/cm^2/s)",
                    min=-18,
                    max=-2,
                    plot_fmt=r"$F_p1$",
                ),
                Parameter(
                    "alpha_1",
                    "temporal decay index of afterglow decay",
                    min=0,
                    max=7,
                    plot_fmt=r"$\alpha_1$",
                ),
                ...
                Parameter(
                    "t1",
                    "log time at the second peak (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t1$",
                ),
#                Parameter(
#                    "t2",
#                    "log boundary time between willingale1 and willingale2 (log sec)",
#                    min=0,
#                    max=10,
#                    plot_fmt=r"$t2$",
#                ),
    ...
    @classmethod
    def POWER_LAW_W07_POWER_LAW(cls):
        return cls(
            name="Simple power law + Willingale 2007 + Simple power law",
            slug="pl+w07+pl",
            func=_pl_w07_pl,
            func_args=[
                Parameter(
                ...
                Parameter(
                    "t2",
                    "log time of afterglow decay (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t2$",
                ),
    ...
    @classmethod
    def W07_W07_POWER_LAW(cls):
        return cls(
            name="Double Willingale 2007 + Simple broken power law",
            slug="w07+w07+pl",
            func=_w07_w07_pl,
            func_args=[
                Parameter(
                ...
                Parameter(
                    "T_p1",
                    "log time at start of first Willingale decay",
                    min=0,
                    max=7,
                    plot_fmt=r"$T_p1$",
                ),
                Parameter(
                    "alpha_p1",
                    "temporal decay index of first Willingale power law",
                    min=0,
                    max=7,
                    plot_fmt=r"$\alpha_p1$",
                ),
                Parameter(
                    "t_p1",
                    "log time of the first Willingale peak (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t_p1$",
                ),
                Parameter(
                    "F_p2",
                    "log flux at peak of second Willingale prompt (log erg/cm^2/s)",
                    min=-18,
                    max=-2,
                    plot_fmt=r"$F_p2$",
                ),
                Parameter(
                    "T_p2",
                    "log time at start of second Willingale decay",
                    min=0,
                    max=7,
                    plot_fmt=r"$T_p2$",
                ),
                Parameter(
                    "alpha_p2",
                    "temporal decay index of second Willingale power law",
                    min=0,
                    max=7,
                    plot_fmt=r"$\alpha_p2$",
                ),
                Parameter(
                    "t_p2",
                    "log time of the second Willingale peak (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t_p2$",
                ),
                Parameter(
                    "F_pl",
                    "log flux at start of power law (log erg/cm^2/s)",
                    min=-18,
                    max=-2,
                    plot_fmt=r"$F_pl$",
                ),
