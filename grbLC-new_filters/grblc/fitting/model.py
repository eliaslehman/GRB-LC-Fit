...
def _w07_w07(x, F_p0, alpha_0, T0, t0, F_p1, alpha_1, T1, t1):
    ...
    if T0 > T1:
        ...
        T0=T1
        T1=tmp
#    F_a = F_p0 + t1 * alpha_1 - t3 * alpha_1 - 10**(t3-t2) / np.log(10)
#    F_p1 = F_a - alpha_1 * (t2 - t1)
#    F_p1 = F_p0 - alpha_1 * (t1 - t2)
    before = lambda x: (
        -10 ** (t0-x) + alpha_0 - alpha_0 * 10 ** (x - T0) + F_p0 * np.log(10)
    ) / np.log(10)
    
    middle = lambda x: F_p0 + T0 * alpha_0 - x * alpha_0 - 10 ** (t0-x) / np.log(10)
#    middle2 = lambda x: (
#        -10 ** (t-x) + alpha_1 - alpha_1 * 10 ** (x - t1) + F_a * np.log(10)
#    ) / np.log(10)
#    middle2 = lambda x: (
#        -10 ** (t-x) + alpha_1 - alpha_1 * 10 ** (x - t2) + F_d * np.log(10)
#    ) / np.log(10)
    middle2 = lambda x: (
        -10 ** (t1-x) + alpha_1 - alpha_1 * 10 ** (x - T1) + F_p1 * np.log(10)
) / np.log(10)
#    after = lambda x: F_a + t1 * alpha_1 - x * alpha_1 - 10 ** (t-x) / np.log(10)
#    after = lambda x: F_d + t2 * alpha_1 - x * alpha_1 - 10 ** (t-x) / np.log(10)
    after = lambda x: F_p1 + T1 * alpha_1 - x * alpha_1 - 10 ** (t1-x) / np.log(10)

    cond = [(x < T0), (x >= T0) * (x < t1), (x >= t1) * (x < T1), (x >= T1)]
    func = [before, middle, middle2, after]
    vals = np.piecewise(x, cond, func)
#    vals=_w07(x, T0, F_p0, alpha_0, t0) + _w07(x, T1, F_p1, alpha_1, t1)
    return vals
...
def _w07_w07_pl(x, F_p, alpha_p, alpha_1, alpha_2, t0, t1, t2, t3, t):
    if t0 > t1:
        tmp=t0
        t0=t1
        t1=tmp
...
#    ) / np.log(10)
    middle2 = lambda x: (
        -10 ** (t-x) + alpha_1 - alpha_1 * 10 ** (x - t1) + F_d * np.log(10)
    ) / np.log(10)

#    middle3 = lambda x: F_a + t1 * alpha_1 - x * alpha_1 - 10 ** (t-x) / np.log(10)
    middle3 = lambda x: F_d + t1 * alpha_1 - x * alpha_1 - 10 ** (t-x) / np.log(10)
#    after = lambda x: F_d - alpha_2 * (x - t2)
    after = lambda x: F_d2 - alpha_2 * (x - t2)
    ...
    def W07_W07_POWER_LAW(cls):
        return cls(
            ...
            func_args=[
                ...
                Parameter(
                    "t1",
                    "log time at end of first W07 (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t1$",
                ),
                Parameter(
                    "t2",
                    "log time at the end of second plateau(log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t2$",
                ),
                Parameter(
                    "t3",
                    "log time of the start of afterglow decay (log sec)",
                    min=0,
                    max=10,
                    plot_fmt=r"$t3$",
                )
